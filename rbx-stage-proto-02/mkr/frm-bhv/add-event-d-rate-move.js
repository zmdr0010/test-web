function addEventDRateMove(setInfo) {
  ...
  const onProgress = (rate) => {
    ...
    const dr = current.dr
    const dc = current.dc
    partInfo.rcInfo.r = Math.round(dr * rate + frmBhvInfo.etcInfo.sr)
    partInfo.rcInfo.c = Math.round(dc * rate + frmBhvInfo.etcInfo.sc)
  }
