const mtxPresetX3List = []

function getMtxPresetX3List() {
  if (mtxPresetX3List.length > 0) return mtxPresetX3List
  for (let i=0; i<2; i++) {
    for (let j=0; j<2; j++) {
      for (let k=0; k<2; k++) {
        for (let l=0; l<2; l++) {
  ...
  return mtxPresetX3List
}

function getMtxX3ListByIndicesListIndex(index, indicesList) {
  const presetList = getMtxPresetX3List()
